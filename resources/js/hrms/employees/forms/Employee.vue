    ...
    <div class="row">
        ...
        <div class="col-md-4 col-sm-12">
            <div class="form-group">
                ...
                <select class="form-control" id="branch_id" name="branch_id" required v-model="BioData.branch_id" :class="{'is-invalid' : BioData.errors.has('branch_id') }">
                    <option value="">--Select Branch--</option>
                    <option v-for="branches in branches" v-bind:key="branches.id" :value="branches.id" >{{branches.name}}</option>
                </select>
            </div>
        </div>
        <div class="col-md-4 col-sm-12">
            <div class="form-group">
                ...
                <select class="form-control" id="department_id" name="department_id" required v-model="BioData.department_id" :class="{'is-invalid' : BioData.errors.has('department_id') }">
        ...
        <div class="col-md-3 col-sm-12">
            <div class="form-group">
                ...
                <input type="official_email" class="form-control" id="official_email" name="official_email" placeholder="Enter Email Address *" required v-model="BioData.official_email" :class="{'is-invalid' : BioData.errors.has('official_email') }">
            </div>
        </div>
    </div>
    <div class="row">
...
export default {
    ...
    mounted() {
        Fire.$on('BioDataFill', employee =>{
            this.employee = employee;
            this.BioData.fill(employee);
            if(employee.user != null){
                this.BioData.first_name = employee.user.first_name;
                this.BioData.middle_name = employee.user.middle_name;
                this.BioData.last_name = employee.user.last_name;
                this.BioData.dob = employee.user.dob;
                this.BioData.sex = employee.user.sex;
                this.BioData.personal_email = employee.user.email;
            } 
            this.BioData.nation_id = employee.nationality_id;
        });
    ...
    methods:{
        createBioData(){
            this.$Progress.start();
            this.BioData.post('/api/hrms/employees')
            .then(response =>{
                this.$Progress.finish();
                Fire.$emit('Reload', response);
                Swal.fire({
                    icon: 'success',
                    title: 'The new staff has been created',
        ...
        updateBioData(){
            this.$Progress.start();
            this.BioData.put('/api/hrms/employees/'+this.BioData.id)
            .then(response =>{
                this.$Progress.finish();
                Fire.$emit('Reload', response);
                Swal.fire({
